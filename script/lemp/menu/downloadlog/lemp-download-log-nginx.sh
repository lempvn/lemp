...
minimumsize=1024000
 checksize=$(du -sb /var/log/nginx/error.log | awk 'NR==1 {print $1}')
	...
	cd /home/$mainsite/private_html/server-log
		sed -i '1s/^/========================================================================= \n\n/' error.log
	sed -i '2s/^/Nginx Log - Created by LEMP \n\n/' error.log
	sed -i '3s/^/========================================================================= \n\n/' error.log

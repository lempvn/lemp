...
# Thêm dòng "Authentication Unique Keys and Salts" nếu chưa tồn tại
#if ! grep -q "Authentication Unique Keys and Salts" /home/$website/public_html/wp-config.php; then
#    sed -i "/.*DB_COLLATE.*/a\\
#    // Authentication Unique Keys and Salts." /home/$website/public_html/wp-config.php
#fi

# Thêm WP_CACHE_KEY_SALT nếu chưa có
#if ! grep -q "define('WP_CACHE_KEY_SALT'" /home/$website/public_html/wp-config.php; then
#    sed -i "/.*Authentication.*/adefine('WP_CACHE_KEY_SALT', '$website');" /home/$website/public_html/wp-config.php
#fi

# Thêm WP_CACHE nếu chưa có
if ! grep -q "define('WP_CACHE'" /home/$website/public_html/wp-config.php; then
    echo "define('WP_CACHE', true);" >> /home/$website/public_html/wp-config.php
fi

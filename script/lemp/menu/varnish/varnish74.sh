...
# https://packagecloud.io/varnishcache/varnish74/packages/ubuntu/bionic/varnish_7.4.3-1~bionic_amd64.deb?distro_version_id=190

/etc/lemp/menu/varnish/install-varnish-begin.sh
echo "7.4" >> /etc/lemp/varnish.version
...
if [ "$OS_VERSION" == "jammy" ]; then
    ...
    echo "Cai dat Varnish 7.4 cho Ubuntu Jammy..."
    sudo apt-get install varnish=7.4.3-1~jammy
elif [ "$OS_VERSION" == "focal" ]; then
    echo "Cai dat kho Varnish cho Ubuntu Focal..."
    curl -s https://packagecloud.io/install/repositories/varnishcache/varnish74/script.deb.sh | sudo bash
    echo "Cai dat Varnish 7.4 cho Ubuntu Focal..."
    sudo apt-get install varnish=7.4.3-1~focal
elif [ "$OS_VERSION" == "bionic" ]; then
    echo "Cai dat kho Varnish cho Ubuntu Bionic..."
    curl -s https://packagecloud.io/install/repositories/varnishcache/varnish74/script.deb.sh | sudo bash
    echo "Cai dat Varnish 7.4 cho Ubuntu Bionic..."
    sudo apt-get install varnish=7.4.3-1~bionic
else
